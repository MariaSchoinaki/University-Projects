public class MaxIntPQ {
    private int[] heap;                                                         // Int Array heap
    private int size;                                                           // To size tou heap
    private static final int DEFAULT_CAPACITY = 4;                              
    private static final int AUTOGROW_SIZE = 4;

    public MaxIntPQ() {                                                         // Kataskeuasths tou MaxIntPQ
        this.heap = new int[DEFAULT_CAPACITY + 1];                              // Dhmiourgeitai to Array
        this.size = 0;                                                          // Arxikopoieitai to size ws 0
    }

    public void insert(int item) {                                       // Methodos insert , item: auto pou theloume na eisagoume
    ...
    public Object peek() {                                         // Methodos peek
    ...
    public Object getmax() {                                       // Methodos getmax
        if (size == 0)                                             // An to size einai 0
            return null;                                           // Epistrefetai null

        Object root = heap[1];                                     // H metablhth root isoutai me to heap[1]
    ...
    private void swim(int i) {                                     // Methodos swim, i: to index tou stoixeiou
        ...
        while (i != 1 && (heap[i]-heap[parent]) > 0) {             // Oso to i einai diaforo tou 1 kai to heap[i]>heap[parent]
    ...
    private void sink(int i) {                                      // Methodos sink
        ...
        while (left <= size) {                                      // Oso left<=size
            ...
            if (right <= size) {                                    // An to right<=size (Yparxei to right)
                if ((heap[left]-heap[right]) < 0)                   // An heap[left]<heap[right]
                    max = right;                                    // Tote to max ginetai to right
            }
            if ((heap[i]-heap[max]) >= 0)                           // An heap[i]>=heap[max]
    ...
    private void swap(int i, int j) {                              // Methodos swap, i,j : index twn stoixeiwn pou theloume na antimetathesoume
    	int tmp = heap[i];                                         // To tmp isoutai me to heap[i]
    ...
    private void grow() {                                           // Methodos grow
        int[] newHeap = new int[heap.length + AUTOGROW_SIZE];       // Dhmiourgeitai neo array

       for (int i = 0; i <= size; i++) {                            // Antigrafontai ta stoixeia tou paliou array sto kainourgio
           newHeap[i] = heap[i];
       }
